...
if (isset($_POST['enviar'])) { //Si se ha pulsado enviar
    ...
    $aErrores['pass2'] = validacionFormularios::comprobarAlfaNumerico($_POST['pass2'], 20, 1, 1); //maximo, mínimo y opcionalidad
    
    if (isset($_POST['nombre']) && isset($_POST['pass']) && isset($_POST['pass2'])) {
        if ($_POST['pass'] === $_POST['pass2']) {
            $codUsuario = $_POST['nombre'];
            $password = $_POST['pass'];
            $consultaSQL1 = "SELECT * FROM Usuario WHERE CodUsuario LIKE '$codUsuario'";
...
if ($entradaOK) {
        ...
        $consultaSQL2 = "INSERT INTO Usuario(CodUsuario, DescUsuario, Password) VALUES (:codigo, :descripcion, SHA2(:pass,256));";
        ...
        $fechaSQL = "UPDATE Usuario SET FechaHoraUltimaConexion = " . time() . " WHERE CodUsuario = :codigo;";
        $actualizarFechaSQL = $miDB->prepare($fechaSQL);
        $actualizarFechaSQL->execute(array(':codigo' => $_POST['nombre']));
            
        $conexionesSQL = "UPDATE Usuario SET NumConexiones = NumConexiones + 1 WHERE CodUsuario = :codigo;";
        $actualizarConexionesSQL = $miDB->prepare($conexionesSQL);
        $actualizarConexionesSQL->execute(array(':codigo' => $_POST['nombre']));
        
        $_SESSION['usuarioDAW213AppLoginLogoff'] = $_POST['nombre'];
        $_SESSION['descUsuario213'] = $_POST['descripcion'];
        $_SESSION['ultimaConexion213'] = null;
        $_SESSION['numConexiones213'] = 1;
